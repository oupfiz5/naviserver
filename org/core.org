...
#+begin_src sh :tangle no :noweb yes
   docker pull oupfiz5/naviserver:4.99.24
#+end_src
...
#+begin_src sh :tangle no :noweb yes
docker build -t oupfiz5/naviserver:4.99.24 -f ./Dockerfile .
#+end_src
...
:END:
 ...
 |-------------------+---------+----------------------------------------|
 | NS_VERSION        | 4.99.24 | Define version for Naviserver          |
 |-------------------+---------+----------------------------------------|
 | NS_MODULE_VERSION | 4.99.24 | Define version for Naviserver  modules |
 |-------------------+---------+----------------------------------------|
...
TCL_BUILD_IMAGE_TAG="${TCL_BUILD_IMAGE_TAG:-23.05}"
UBUNTU_IMAGE_REPOSITORY=""
UBUNTU_IMAGE_NAME="${UBUNTU_IMAGE_NAME:-ubuntu}"
UBUNTU_IMAGE_TAG="${UBUNTU_IMAGE_TAG:-22.04}"
#+end_src
